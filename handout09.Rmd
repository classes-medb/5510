...
Keep in mind that Cronbach's alpha and KR-20 only work for composite measures. If you are measuring pain using the Faces scale, that's a single number 1 through 5. It is single item so there is nothing internal to be consistent about. If you still need to assess reliability, and that would probably mean something like test-retest reliability. 

As another example, the Clark scale (details are [here](https://www.cancerresearchuk.org/about-cancer/melanoma/stages-types/clark-breslow-staging))is a measure used in melanoma cases. It is a single item with five levels describing how deeply a melanoma has penetrated the skin. You can't use alpha because on a single item scale. Since this is a physician report, your main concern about reliability would be assuring that two different physicians would, more often than not, use the same level when they are looking at the same melanoma. The intra-class correlation coefficient is your best bet here.